...
import com.quna.rag.util.TextCleaningUtil;
...
public class CommonDocumentParser implements DocumentParser {
    ...
    public ParsedDocument parse(MultipartFile file) throws Exception {
        ...
        // 处理空文件
        if (file.isEmpty() || file.getSize() == 0 || StringUtils.isEmpty(fileName)) {
        ...
        return new ParsedDocument(file.getOriginalFilename(), FileValidationUtil.extension(file.getOriginalFilename()), text.trim(), true);
    }
    ...
    private String parsePoiDocument(MultipartFile file) {
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            String cleanedContent = TextCleaningUtil.cleanText(content);
